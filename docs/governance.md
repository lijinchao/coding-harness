...
A harness can pass every gate and still be wrong about itself: a README naming an old version, a decision record missing an alternative, an owner nobody routes to. `harness doctor --manifest <path>` checks the facts the manifest declares under `governance` — version files, decision-record and change-record sections, owner routing, the declared CI file's harness commands, the existence of a declared manual-verification path, the links and budgets of declared documents, and the completeness of the declared instruction tree — so governance-fact drift fails a gate instead of surviving on trust. A recommended key that is absent is a warning, not a failure. The [writing-a-gate](../base/skills/writing-a-gate/SKILL.md) skill owns the procedure.
