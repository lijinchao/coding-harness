...
A harness can pass every gate and still be wrong about itself: a README naming an old version, a decision record missing an alternative, an owner nobody routes to. `harness doctor --manifest <path>` checks the facts the manifest declares under `governance` — version files, decision-record sections, owner routing, and the declared CI file's harness commands — so governance-fact drift fails a gate instead of surviving on trust. The [writing-a-gate](../base/skills/writing-a-gate/SKILL.md) skill owns the procedure.
