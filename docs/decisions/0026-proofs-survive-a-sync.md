...
- A recorded proof survives sync, upgrade, and a proof's own revert, so `prove` can run repeatedly; `test/scan-prove.test.mjs` covers `prove --record`, commit, `prove`, clean tree.
