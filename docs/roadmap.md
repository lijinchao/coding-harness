...
Progress: `harness init` scaffolds the delta, manifest, bootstrap, `.gitignore`, and a CI workflow, then composes the repository (green when the base source resolves). `harness diff` previews a base upgrade. `harness gates` supports `--jobs` and `--timeout`, and `--report` feeds `harness metrics` (first-pass rate). Remaining: YAML manifest support, and a `harness bump --to <version>` command that performs the version choreography (base pin, tool pin, `base@`/`v` references, `governance.version` files, lock) in one step. The manual version edit has now been missed twice and `doctor` caught it twice; the correction belongs in a command, not in discipline.
