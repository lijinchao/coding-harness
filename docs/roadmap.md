...
Status: v0.1.39 — the released kernel remains unchanged; the working tool surveys repositories, checks pinned multi-repository snapshots, runs reviewed tiers into portable receipts, applies receipt age budgets, and measures bounded CI Shadow stability. External-qualified execution and CI enforcement remain separately authorized. Base and tool releases are versioned, hashed, and commit-pinned; the manifest carries gates, governance, changed-surface selection, proof records, metrics, packs, legibility declarations, and eval declarations. The detailed current contracts live in [reference.md](reference.md) and [contracts.md](contracts.md).
