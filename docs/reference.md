...
A manifest may pin `tool.version`. A committed bootstrap fetches the tool at tag `v<version>` and runs it; the lock records the tool version and a SHA-256 per tool source file, and a running tool that differs from either fails. `harness gates --report <file>` appends one line per run: the time, the versions, and each gate's id, verdict, timeout, and duration. `harness metrics --log <file>` reports the first-pass rate, duration percentiles, and the gates that fail, skip, time out, or flake; given `--manifest`, it judges the declared budget over the last `window` runs of `governance.metrics.log` — `minFirstPassRate`, `maxFlaky`, `maxTimeouts` — and exits non-zero on a breach. `exclude` names gate ids the window ignores, which is how a judging gate keeps its own verdict out of its evidence. Pointing `--report` at the committed log is what grows the window. Duration is reported, not budgeted: the same gate times differently on different runners, so a p95 threshold would fail on hardware, not on a regression.

The lock records the declared pin: `lock.tool.commit` is the manifest's `tool.commit`, and `check` fails when the two disagree, so a lock written by another build cannot pass as this one. A manifest that pins `tool.version` without `tool.commit` leaves a moved tag undetected. CI needs no checkout of the tool repository. A repository's own version is separate from the base pin. `product.version.path` names the one file that holds it, with an optional `pattern` whose first capture group is the version; without one the file must be exactly a version. `product.mentions` lists the documents that must agree, and `doctor` fails a mention that does not. The base pin lives only in the manifest and the lock: it is a dependency, and prose never restates it.

`harness init` writes `tool.commit` from the running tool, so a scaffolded repository pins the commit as well as the version and the bootstrap rejects a moved tag. `harness init --base-source <dir>` accepts a local release registry and pins `tool.source` to the running tool's checkout, so a local scaffold needs no fetch. A local `tool.source` names a directory that contains `bin/harness.mjs`; the bootstrap also accepts a path below that directory.
...
The bootstrap resolves the manifest and the tool cache next to the script, so invoking it by absolute path from any directory pins the same tool. `harness sync` writes the bootstrap from the running tool and `check` fails when it is missing or differs, so a bootstrap fix reaches a repository already initialized.
...
`harness doctor --manifest <path>` checks the facts the manifest declares: the product version source and the documents that must agree with it, the sections and lifecycle status of each decision record, the sections and `Decision:` reference of each change record, a CODEOWNERS route for each declared owner, each declared CI file's required commands, and a recorded proof naming a gate the manifest no longer declares; `harness prove` owns freshness: it refuses a gate whose record is missing or stale. It also reports the adoption gap against the pinned release.

`harness gates --manifest <path>` runs every gate the manifest declares, so one list drives local runs and CI and the two cannot drift. A blocking failure exits non-zero; an advisory failure only warns. `harness scan --root <dir>` reports every manifest under a tree as ok, stale, diverged, or error. `harness prove --manifest <path> [--isolated]` runs a gate's `prove_fires_command`, requires the gate to fail, reverts, and requires it to pass; a gate whose action no longer fires exits non-zero. `--isolated` proves in a temporary worktree and refuses a base source that is a relative path outside git, which the worktree cannot resolve. The worktree shares `.harness`, `node_modules`, and each `governance.proofCarry` path with the repository, so a proof that modifies or removes one of those files fails and names it, while a cache that merely grows is reported.
