...
test('packs lists what a declared pack contributes and what the repository overrode', () => {
  const root = mkdtempSync(join(tmpdir(), 'coding-harness-pack-list-'))
  const registry = makePack(root, packJson())
  const consumer = makeConsumer(root, registry, { gates: [gate('demo/architecture', { override: true })] })
  const manifest = join(consumer, 'harness.manifest.json')
  run(['sync', '--manifest', manifest])
  const listed = run(['packs', '--manifest', manifest])
