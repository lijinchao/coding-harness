...
- Re-record the example after changing `base/`: `node bin/harness.mjs sync --manifest examples/consumer/harness.manifest.json`
...
- `examples/consumer/` is a generating repository. Change `base/`, then re-run `sync`; never hand-edit `examples/consumer/AGENTS.md` or `REVIEW.md`.
